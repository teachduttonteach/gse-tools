  export class Utilities {
    ...
    compareDates(
      ...
      const yearComp: number = date1.getUTCFullYear() > date2.getUTCFullYear() ? 1 : (date1.getUTCFullYear() < date2.getUTCFullYear() ? -1 : 0);
      if (level == "YEAR" && yearComp != 0) return this._checkDate(yearComp, greaterThan);
      const monthComp: number = date1.getUTCMonth() > date2.getUTCMonth() ? 1 : (date1.getUTCMonth() < date2.getUTCMonth() ? -1 : 0);
      if (level != "DAY" && monthComp != 0) return this._checkDate(monthComp, greaterThan);
      const dayComp: number = date1.getUTCDate() > date2.getUTCDate() ? 1 : (date1.getUTCDate() < date2.getUTCDate() ? -1 : 0);
      return this._checkDate(dayComp, greaterThan);

    }

  _checkDate(comp: number, greaterThan: boolean): boolean {
      if ((greaterThan && comp == 1) || (!greaterThan && comp == -1)) return true;
      return false;
  }
}